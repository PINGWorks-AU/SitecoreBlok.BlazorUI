...
| TimePicker | ![Parity](https://img.shields.io/badge/Parity-22c55e?style=flat-square) | [`time-picker.tsx`](https://github.com/Sitecore/blok/blob/main/src/components/ui/time-picker.tsx) | `931987` | Trigger button plus a popover with Hour / Minute / Period selects and Clear / Done, matching Blok. Value stays `TimeSpan?` rather than Blok's `{hour, minute, period}` string triple. Requires a root `<Popovers />` |
